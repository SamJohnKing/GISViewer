...
public class MapWizard extends JFrame implements ActionListener{
	...
	JMenuItem OpenItem,ExitItem,BasicInfoItem,SaveItem,TwoPointItem,ClearAllStaticPoint,ClearLastPoint,ClearDirection;
	JMenuItem CalibrateItem,ShowClockItem,ShowTaxiSearchItem,RouteSearchItem,WashScreenItem,ShowCenterItem,VeilCenterItem;
	JMenuItem LandMarkEditItem,AboutFrameItem,LandMarkOnScreenItem,LandMarkVeilItem,LandMarkNameOnScreenItem,LandMarkNameVeilItem;
	...
	double LongitudeStart=1e100,LongitudeEnd=-1e100,LatitudeStart=1e100,LatitudeEnd=-1e100;
	JFrame SubFrameClock;
	public GPSSet GPSPoints;
	int HighestRate;
	double RoadDeltaLongitude,RoadDeltaLatitude,GPSDeltaLongitude,GPSDeltaLatitude;
	NULLPaneClass NULL;
...
//---------------------------------------------------------------------------
	public class ConcentrateTaxiWizardClass extends JFrame{
		...
		JButton ShowTaxiCode,VeilTaxiCode,StartTrace,GiveUpTrace,ShowLandMark,Back;
		JLabel l0;
		JTextField Code;
		...
		public void actionPerformed(ActionEvent e){
			if(e.getSource()==ShowTaxiCode){
				...
				ShowTaxiCode.setEnabled(true);
				VeilTaxiCode.setEnabled(false);
			}else if((e.getSource()==Code)||(e.getSource()==StartTrace)){
				int kkk=-1;
...
//-----------------------------------------------------------------------
	public class LandMarkSpotFrameClass extends JFrame{//显示地标点的快速定位窗口
		...
		class Photo extends Canvas{//专门用来显示图片
			...
			public void paint(Graphics g){
				...
				if(ImageDir!=null){
					...
					if(f.exists()){
						img=kit.getImage(ImageDir.toString()+"/"+PhotoName);
					}
				}
		...
		public FacePic(){
			...
			add(new JScrollPane(Script,ScrollPaneConstants.VERTICAL_SCROLLBAR_AS_NEEDED,ScrollPaneConstants.HORIZONTAL_SCROLLBAR_NEVER));
			add(WriteBack);
			add(Hide);
			add(Confirm);
		}
		...
		public void actionPerformed(ActionEvent e){
			if(e.getSource()==Hide){
				...
				GPSPoints.LandMarkScript[Emerge_ID]=Script.getText().trim();
				Hide();
			}
		}
		public void mouseDragged(MouseEvent e) {
...
public class LandMarkQueryFrameClass extends JFrame{//用于显示数据库的显示窗口
	FacePic Pic;
	class FacePic extends JPanel implements ActionListener,MouseListener,MouseMotionListener{
	JButton Hide,Query,DeleteRow,Delete,UpdateRow,Update,MoreInfo,Transit,TransitAll;
	JLabel l0,l1,l2;
	JTextField Name,Type;
	...
	public void reQuery(){
		...
		for(int i=0;i<GPSPoints.LandMarkNum;i++){
			...
			temp[1]=java.lang.Double.toString(GPSPoints.LandMarkLongitude[i]);
			temp[2]=java.lang.Double.toString(GPSPoints.LandMarkLatitude[i]);
	...
	public void actionPerformed(ActionEvent e){
		...
		if(e.getSource()==Hide){
			...
			if(selectRows==1){
				int selectedRowIndex = ResultTable.getSelectedRow();
				int index=Integer.parseInt((String)tableModel.getValueAt(selectedRowIndex,0));
			...
			for(int i=0;i<RowCount;i++){
			  String str=(String)tableModel.getValueAt(i,0);
			  try{
				String sss="[Title:"+GPSPoints.LandMarkName[Integer.parseInt(str)]+"][Info:"+GPSPoints.LandMarkType[Integer.parseInt(str)]+"]";
...
public void ClearStateAfterSwitchPane(){//当工具栏切换时进行清空屏幕的善后工作，并且为新面板初始化
	...
	for(int i=0;i<GPSPoints.LandMarkNum;i++) Screen.IsLandMarkSelected[i]=false;
	if(Screen.lock){
...
//------------------------------------------------------
	class CalibratePaneClass extends ToolPanel implements ActionListener{
		...
		JTextField AveDeltaLongitude,AveDeltaLatitude,AveDeltaNorth,AveDeltaEast;
		JRadioButton SetPoint1,SetPoint2,GoalPoint1,GoalPoint2;
		JButton ForbidAddPoint,PermitAddPoint;
		JButton RemoveSelectedPoints,RemoveInvolvedPoints,ShowDirection,ShowResult;
		boolean CanAdd=true;
		public void paintComponent(Graphics g){
			Toolkit kit=getToolkit();
			...
			if(e.getSource()==ScreenLockButton){
				...
				for(int i=0;i<Screen.XYCount;i++){
					if(i%2==1){
						...
						dx=x2-x1;
						dy=y2-y1;
						disx=(x2-x1)/180*Math.PI*6371*1000*Math.cos((y1+y2)/360*Math.PI);
	...
	class BasicInfoPaneClass extends ToolPanel implements ActionListener{
		...
		public void actionPerformed(ActionEvent e){
			if(e.getSource()==ScreenLockButton){
				...
				if(Screen.LandMarkSelectedNum==0) return;
				if(SlideID==-1) return;
					int temp=-1;
...
//--------------------------------------------------
	class TaxiSearchPaneClass extends ToolPanel implements ActionListener{
		...
		JTextField LongitudeText,LatitudeText,CenterLongitude,CenterLatitude,RadiusDis,TaxiCount,TheNearestDis;
		JButton ShowSelectedTaxi,NotShowSelectedTaxi,TraceTheNearest,StopTrace,DataBaseReference,TraceFree;
		...
		public void GetGPSInfo(){
			...
			GPSPoints.TaxiHeapPop(tempHour*3600+tempMinute*60+tempsecond);
			for(int i=0;i<GPSPoints.TaxiNum;i++) IsShowTaxi[i]=false;
			...
			if(ConcentrateTaxiWizard.IsShowLandMark){
				...
				for(int i=0;i<GPSPoints.LandMarkNum;i++){
					tempdis=GPSPoints.Distance(GPSPoints.LandMarkLongitude[i],GPSPoints.LandMarkLatitude[i],xx,yy);
					if(tempdis<mindis){
						mindis=tempdis;
						best=i;
					}
		...
		public void actionPerformed(ActionEvent e){
			if(e.getSource()==ScreenLockButton){
				...
				try{
					centerlongitude=java.lang.Double.parseDouble(CenterLongitude.getText());
				...
				Screen.XYCount=0;
				FreshIsShowTaxi();
				double w=2*centerradius/6371/1000/Math.cos(centerlatitude/180*Math.PI)/Math.PI*180;
				...
				if(ShowSelectedTaxi.isEnabled()) return;
				if(ConcentrateTaxiWizard.isVisible())
				ConcentrateTaxiWizard.Hide();
				else ConcentrateTaxiWizard.emerge();
			}
		}
	}
//------------------------------------------------
	...
	//实现两点定位工具栏的功能，测量地图上两点的球面距离
			...
			JTextField LongitudeText,LatitudeText,X1Text,Y1Text,X2Text,Y2Text;
			JTextField DeltaLongitude,DeltaLatitude,DeltaNorth,DeltaEast,DeltaDis;
			JRadioButton SetPoint1,SetPoint2,GoalPoint1,GoalPoint2;
			...
			public void actionPerformed(ActionEvent e){
				if(e.getSource()==ScreenLockButton){
					...
					try{
					double tempx1=java.lang.Double.parseDouble(X1Text.getText());
					double tempy1=java.lang.Double.parseDouble(Y1Text.getText());
					double tempx2=java.lang.Double.parseDouble(X2Text.getText());
					double tempy2=java.lang.Double.parseDouble(Y2Text.getText());
					...
					double delta_north=delta_latitude/180*Math.PI*6371*1000;
					double delta_east=delta_longitude/180*Math.PI*6371*1000*Math.cos((tempy1+tempy2)/360*Math.PI);
					double delta_dis=Math.sqrt(delta_north*delta_north+delta_east*delta_east);
					
					DeltaLongitude.setText(java.lang.Double.toString(delta_longitude));
					DeltaLatitude.setText(java.lang.Double.toString(delta_latitude));
					...
					Screen.repaint();
					Idle(2000);
					Screen.MiddleReSize(Math.abs(tempx2-tempx1), Math.abs(tempy2-tempy1));
					Screen.repaint();
					CanChange=true;
					}catch(Exception ex){
						JOptionPane.showMessageDialog(null,"您的经纬度输入不正确","数据安全提示",JOptionPane.WARNING_MESSAGE);
...
//--------------------------------------
	 class RouteSearchPaneClass extends ToolPanel implements ActionListener,ItemListener{
		 public String getString(){
	...
	//为用户提供路径规划的功能，需要读取和处理底层的路网数据
			...
			JTextField LongitudeText,LatitudeText,SourceLongitude,SourceLatitude,TerminalLongitude,TerminalLatitude,PathNum,PathLength;
			...
			void DrawRouteOnScreen(int k){
				...
				len+=GPSPoints.Distance(Screen.LineXlist[i-1],Screen.LineYlist[i-1],Screen.LineXlist[i],Screen.LineYlist[i]);
...
//--------------------------------------
		class LandMarkEditPaneClass extends ToolPanel implements ActionListener{
			...
			JTextField LongitudeText,LatitudeText,LandMarkLongitude,LandMarkLatitude;
			...
			public void actionPerformed(ActionEvent e){
				if(e.getSource()==ScreenLockButton){
					...
					int tot=0;
					for(int i=0;i<GPSPoints.LandMarkNum;i++){
						if(Screen.IsLandMarkSelected[i]) continue;
...
//--------------------------------------
	public class ScreenCanvas extends Canvas implements MouseListener,MouseWheelListener,MouseMotionListener{
		...
		double LargeRegionLongitude,LargeRegionLatitude,LargeRegionLongitudeScale,LargeRegionLatitudeScale;
		...
		double showDirectionX1,showDirectionX2,showDirectionY1,showDirectionY2;
		...
		public boolean CheckInScreen(int x1,int y1,int x2,int y2){
			if(Math.max(Math.min(x1,x2),0)>Math.min(Math.max(x1,x2),ScreenWidth)) return false;
			if(Math.max(Math.min(y1,y2),0)>Math.min(Math.max(y1,y2),ScreenHeight)) return false;
			return true;
		}
		...
		public void ExtendedPointSelectDelete(){
			...
			for(int i=1;i<ExtendedPointSelectCount;i++){
				temp=ExtendedPointMoveBack(ExtendedPointSelectList[i-1]+1,ExtendedPointSelectList[i]-1,temp);
			}
			temp=ExtendedPointMoveBack(ExtendedPointSelectList[ExtendedPointSelectCount-1]+1,ExtendedPointCount-1,temp);
			ExtendedPointCount=temp;
			ExtendedPointSelectCount=0;
		}
		public void ExtendedPointSelect(double x1,double y1,double x2,double y2){
			ExtendedPointSelectCount=0;
			for(int i=0;i<ExtendedPointCount;i++){
				if(Handle.AccurateInsideRectangle(ExtendedPointX[i],ExtendedPointY[i], x1, y1, x2, y2)){
		...
		public void ExtendedPointReHint(int k,String Hint){
			if(k<0) return;
			if(k>=ExtendedPointCount) return;
		...
		private int setAlignTagVector(Database.LineDataSet LineDB,String str,int index,double scale){
			...
			while(index!=-1){
				...
				Handle.AccurateDistance(AlignVectorX[AlignVectorCounter-1],AlignVectorY[AlignVectorCounter-1],LineDB.AllPointX[index],LineDB.AllPointY[index]);
				AlignVectorCounter++;
				index=LineDB.AllPointNext[index];
			}
			double lenspan=OffsetCounter[AlignVectorCounter-1]*3/4/str.length();
			double lenpos=OffsetCounter[AlignVectorCounter-1]/8+lenspan/2;
			double screenwidth=lenspan*scale;
			int ptr=0;
			for(int i=0;i<str.length();i++,lenpos+=lenspan){
				while(OffsetCounter[ptr+1]<lenpos) ptr++;
				AlignPosX[i]=AlignVectorX[ptr]+(AlignVectorX[ptr+1]-AlignVectorX[ptr])*(lenpos-OffsetCounter[ptr])/(OffsetCounter[ptr+1]-OffsetCounter[ptr]);
				AlignPosY[i]=AlignVectorY[ptr]+(AlignVectorY[ptr+1]-AlignVectorY[ptr])*(lenpos-OffsetCounter[ptr])/(OffsetCounter[ptr+1]-OffsetCounter[ptr]);
			}
			return Math.min(16,(int)(screenwidth/str.length()*2));
		}
		//--------------------------------------------------------------------
		private double VectorYmax,VectorYmin,VectorXmax,VectorXmin,VectorX,VectorY;
		private double[] CenterPosX=new double[10000];
		private double[] CenterPosY=new double[1000];
		private int setCenterTagVector(Database.PolygonDataSet PolyDB,String str,int index,double scale){
			...
			double offsetdis=Handle.AccurateDistance(VectorXmin,VectorYmin,VectorXmax,VectorYmax);
			...
			if((VectorYmax-VectorYmin)/(VectorXmax-VectorXmin)<1.25){
				for(int i=0;i<str.length();i++,lenpos+=lenspan){
					CenterPosX[i]=VectorXmin+lenpos/offsetdis*(VectorXmax-VectorXmin);
					CenterPosY[i]=(VectorYmax+VectorYmin)/2;
				}
			}else{
				for(int i=0;i<str.length();i++,lenpos+=lenspan){
					CenterPosX[i]=VectorXmax-lenpos/offsetdis*(VectorXmax-VectorXmin);
					CenterPosY[i]=VectorYmax-lenpos/offsetdis*(VectorYmax-VectorYmin);
				}
			}
		...
		public ScreenCanvas(){//初始化各种数据
			...
			for(int i=0;i<10000;i++) IsLandMarkSelected[i]=false;
		}
		double Xlist[],Ylist[];
		int XYCount;
		...
		int[][] AlphaGridsCounter=new int[1000][1000];
		...
		public void paint(Graphics g){			
			...
			g.drawImage(image, 0, 0,ScreenWidth,ScreenHeight,(int)Xst,(int)Yst,(int)(Xst+Xlen),(int)(Yst+Ylen),this);
			}
			...
			boolean Selected;			
			//画动态的临时点
			double tempsize=0.33-LongitudeScale/(LongitudeEnd-LongitudeStart);
			...
			for(int i=0;i<XYCount;i++){
				...
				if(j<=SelectedPointList[0]){
					if(i<SelectedPointList[j]){}
						...
						Selected=true;
						j++;
					}else if(i>SelectedPointList[j]) j++;
				}
				if(Selected) g_2d.setColor(Color.blue);
				Ellipse2D pp=new Ellipse2D.Double(xx-4+ScreenDeltaX,yy-4+ScreenDeltaY,8,8);
				g_2d.draw(pp);
				if(Selected) g_2d.setColor(Color.green);
				//显示出租车编号
				if(NowPanel==TaxiSearchPane){
					if(ConcentrateTaxiWizard.VeilTaxiCode.isEnabled()){
						g_2d.drawString(TempString[i], xx+8+ScreenDeltaX, yy-8+ScreenDeltaY);
					}
				}
			}
			//画大圆形
			if(ShowLargeRegion){
				int x0=(int)((LargeRegionLongitude-ScreenLongitude)/LongitudeScale*ScreenWidth);
				int y0=(int)((ScreenLatitude-LargeRegionLatitude)/LatitudeScale*ScreenHeight);
				...
				if(ClockWizard.pic.getSecond()%2==0) g_2d.setColor(Color.blue);
				else g_2d.setColor(Color.red);
				g_2d.fillOval(x0-6,y0-6,12,12);
				Ellipse2D pp=new Ellipse2D.Double(x0-w/2.0+ScreenDeltaX,y0-h/2.0+ScreenDeltaY,w,h);
				g_2d.draw(pp);
				g_2d.setColor(Color.green);
			}
			//画箭头
			if(IsShowDirection){
				...
				int x1=(int)((showDirectionX1-ScreenLongitude)/LongitudeScale*ScreenWidth);
				int y1=(int)((ScreenLatitude-showDirectionY1)/LatitudeScale*ScreenHeight);
				int x2=(int)((showDirectionX2-ScreenLongitude)/LongitudeScale*ScreenWidth);
				int y2=(int)((ScreenLatitude-showDirectionY2)/LatitudeScale*ScreenHeight);
				Line2D l1=new Line2D.Double(x1+ScreenDeltaX,y1+ScreenDeltaY,x2+ScreenDeltaX,y2+ScreenDeltaY);
				int x3=(x1+x2)/2+(y2-y1)/4;
				int y3=(y1+y2)/2-(x2-x1)/4;
				x3=x3+3*(x2-x3)/4;
				y3=y3+3*(y2-y3)/4;
				Line2D l2=new Line2D.Double(x3+ScreenDeltaX,y3+ScreenDeltaY,x2+ScreenDeltaX,y2+ScreenDeltaY);
				x3=(x1+x2)/2-(y2-y1)/4;
				y3=(y1+y2)/2+(x2-x1)/4;
				x3=x3+3*(x2-x3)/4;
				y3=y3+3*(y2-y3)/4;
				Line2D l3=new Line2D.Double(x3+ScreenDeltaX,y3+ScreenDeltaY,x2+ScreenDeltaX,y2+ScreenDeltaY);
				g_2d.draw(l1);
				g_2d.draw(l2);
				g_2d.draw(l3);
					...
					for(int i=0;i<XYCount;i++){
						if(i%2==1){
							int x1=(int)((Xlist[i-1]-ScreenLongitude)/LongitudeScale*ScreenWidth);
							int y1=(int)((ScreenLatitude-Ylist[i-1])/LatitudeScale*ScreenHeight);
							int x2=(int)((Xlist[i]-ScreenLongitude)/LongitudeScale*ScreenWidth);
							int y2=(int)((ScreenLatitude-Ylist[i])/LatitudeScale*ScreenHeight);
							x1+=ScreenDeltaX;
							y1+=ScreenDeltaY;
							x2+=ScreenDeltaX;
							y2+=ScreenDeltaY;
					...
					int x1=(int)((Xlist[0]-ScreenLongitude)/LongitudeScale*ScreenWidth);
					int y1=(int)((ScreenLatitude-Ylist[0])/LatitudeScale*ScreenHeight);
					int x2=(int)((Xlist[1]-ScreenLongitude)/LongitudeScale*ScreenWidth);
			...
			if(LineCount!=0){
				...
				if(RouteSearchPane.PathOnScreen){
					...
					for(int i=1;i<LineCount;i++){
						int x1=(int)((LineXlist[i-1]-ScreenLongitude)/LongitudeScale*ScreenWidth);
						int y1=(int)((ScreenLatitude-LineYlist[i-1])/LatitudeScale*ScreenHeight);
						int x2=(int)((LineXlist[i]-ScreenLongitude)/LongitudeScale*ScreenWidth);
						int y2=(int)((ScreenLatitude-LineYlist[i])/LatitudeScale*ScreenHeight);
						x1+=ScreenDeltaX;
						y1+=ScreenDeltaY;
						x2+=ScreenDeltaX;
						y2+=ScreenDeltaY;
			...
			if(CanDrawRect){//画矩形
				Rectangle2D rect=new Rectangle2D.Double(DrawRectX1+ScreenDeltaX,DrawRectY1+ScreenDeltaY,DrawRectX2-DrawRectX1,DrawRectY2-DrawRectY1);
				g_2d.draw(rect);
			}
			if(ShowCenter){//画中心选中标识
				g_2d.setColor(Color.red);
				Ellipse2D rect=new Ellipse2D.Double(ScreenWidth/2-15+ScreenDeltaX,ScreenHeight/2-15+ScreenDeltaY,30,30);
				g_2d.draw(rect);
				bs=new BasicStroke(2,BasicStroke.CAP_ROUND,BasicStroke.JOIN_ROUND);
				g_2d.setStroke(bs);
				g_2d.setColor(Color.green);
			...
			if(IsLandMarkOnScreen){//显示地标点，静态点
				for(int i=0;i<GPSPoints.LandMarkNum;i++){
					...
					int x0=(int)((GPSPoints.LandMarkLongitude[i]-ScreenLongitude)/LongitudeScale*ScreenWidth);
					int y0=(int)((ScreenLatitude-GPSPoints.LandMarkLatitude[i])/LatitudeScale*ScreenHeight);
					x0+=ScreenDeltaX;
					y0+=ScreenDeltaY;
					g_2d.fillRect(x0-3,y0-3,6,6);
					if(IsLandMarkNameOnScreen){
						g_2d.drawString(GPSPoints.LandMarkName[i], x0+8, y0-8);
					}
				}
				g_2d.setColor(Color.green);
			}
			...
			if((IsExtendedPointVisible)&&(ExtendedPointCount>0)){
				...
				g_2d.setFont(new Font("黑体",0,(int )(90*tempsize)));
				for(int i=0;i<ExtendedPointCount;i++){
					int xx=(int)((ExtendedPointX[i]-ScreenLongitude)/LongitudeScale*ScreenWidth);
					int yy=(int)((ScreenLatitude-ExtendedPointY[i])/LatitudeScale*ScreenHeight);
					xx+=ScreenDeltaX;
					yy+=ScreenDeltaY;
				...
				for(int i=1;i<ExtendedPointCount;i++){
					int x1=(int)((ExtendedPointX[i-1]-ScreenLongitude)/LongitudeScale*ScreenWidth);
					int y1=(int)((ScreenLatitude-ExtendedPointY[i-1])/LatitudeScale*ScreenHeight);
					int x2=(int)((ExtendedPointX[i]-ScreenLongitude)/LongitudeScale*ScreenWidth);
					int y2=(int)((ScreenLatitude-ExtendedPointY[i])/LatitudeScale*ScreenHeight);
					x1+=ScreenDeltaX;
					y1+=ScreenDeltaY;
					x2+=ScreenDeltaX;
					y2+=ScreenDeltaY;
				...
				if(IsHeadTailLink){
					g_2d.setColor(Color.red);
					int x1=(int)((ExtendedPointX[0]-ScreenLongitude)/LongitudeScale*ScreenWidth);
					int y1=(int)((ScreenLatitude-ExtendedPointY[0])/LatitudeScale*ScreenHeight);
					int x2=(int)((ExtendedPointX[ExtendedPointCount-1]-ScreenLongitude)/LongitudeScale*ScreenWidth);
					int y2=(int)((ScreenLatitude-ExtendedPointY[ExtendedPointCount-1])/LatitudeScale*ScreenHeight);
					x1+=ScreenDeltaX;
					y1+=ScreenDeltaY;
					x2+=ScreenDeltaX;
					y2+=ScreenDeltaY;
				...
				if(ExtendedPointSelectCount!=0){
					for(int i=0;i<ExtendedPointSelectCount;i++){
						int xx=(int)((ExtendedPointX[ExtendedPointSelectList[i]]-ScreenLongitude)/LongitudeScale*ScreenWidth);
						int yy=(int)((ScreenLatitude-ExtendedPointY[ExtendedPointSelectList[i]])/LatitudeScale*ScreenHeight);
						xx+=ScreenDeltaX;
						yy+=ScreenDeltaY;
						g_2d.setColor(Color.red);
						Ellipse2D pp=new Ellipse2D.Double(xx-4,yy-4,8,8);
			...
			if(IsAllElementInvisible) return;
			//Draw for Extended Database--------------------------
			SetAREA(ScreenLongitude,ScreenLongitude+LongitudeScale,ScreenLatitude-LatitudeScale,ScreenLatitude);
			if(!IsAllLineInvisible)
			if(LineDatabaseFile!=null){
				int binary,choose,now,p1,p2;
				Line2D line;
				tempsize=0.2-LongitudeScale/(LongitudeEnd-LongitudeStart);
				tempsize=tempsize<=0?0:tempsize;
				...
				for(int i=0;i<LineDatabase.LineNum;i++){
					...
					if((binary&Ox("1"))!=0){
						if(LineDatabase.LineHint[i].indexOf("[Info:Cache]")!=-1)
						if(LineDatabase.LineHint[i].indexOf("[Info:Path]")!=-1){
							bs=new BasicStroke(10,BasicStroke.CAP_ROUND,BasicStroke.JOIN_ROUND);
							g_2d.setStroke(bs);
						}
						if((binary&Ox("1000"))!=0){//For Line----------------------------
							...
							if(p2==-1) break;
							if(!CheckInAREA(LineDatabase.AllPointX[p1],LineDatabase.AllPointY[p1],LineDatabase.AllPointX[p2],LineDatabase.AllPointY[p2])){
								p1=p2;
								continue;
							}
							int x1=(int)((LineDatabase.AllPointX[p1]-ScreenLongitude)/LongitudeScale*ScreenWidth);
							int y1=(int)((ScreenLatitude-LineDatabase.AllPointY[p1])/LatitudeScale*ScreenHeight);
							int x2=(int)((LineDatabase.AllPointX[p2]-ScreenLongitude)/LongitudeScale*ScreenWidth);
							int y2=(int)((ScreenLatitude-LineDatabase.AllPointY[p2])/LatitudeScale*ScreenHeight);
							x1+=ScreenDeltaX;
							y1+=ScreenDeltaY;
							x2+=ScreenDeltaX;
							y2+=ScreenDeltaY;
							line=new Line2D.Double(x1,y1,x2,y2);
							if(CheckInScreen(x1,y1)||CheckInScreen(x2,y2)||CheckInScreen(x1,y1,x2,y2)){
							g_2d.draw(line);
							DrawCount++;
							}
							p1=p2;
						...
						if((binary&Ox("100"))!=0){//For Point-------------------------
							choose=(binary>>7)&Ox("111");
							g_2d.setColor(getChooseColor(choose));
							now=LineDatabase.LineHead[i];
							while(now!=-1){
								if(!CheckInAREA(LineDatabase.AllPointX[now],LineDatabase.AllPointY[now])){
									now=LineDatabase.AllPointNext[now];
									continue;
								}
								int xx=(int)((LineDatabase.AllPointX[now]-ScreenLongitude)/LongitudeScale*ScreenWidth);
								int yy=(int)((ScreenLatitude-LineDatabase.AllPointY[now])/LatitudeScale*ScreenHeight);
								xx+=ScreenDeltaX;
								yy+=ScreenDeltaY;
								if(CheckInScreen(xx,yy)){
								g_2d.fillOval(xx-3, yy-3, 6, 6);
								DrawCount++;
								}
								now=LineDatabase.AllPointNext[now];
							}
						}
						if((binary&Ox("10"))!=0){//For Word--------------------------
							choose=(binary>>4)&Ox("111");
							g_2d.setColor(getChooseColor(choose));
							now=LineDatabase.LineHead[i];
							int xx=(int)((LineDatabase.AllPointX[now]+LineDatabase.dx[i]-ScreenLongitude)/LongitudeScale*ScreenWidth);
							int yy=(int)((ScreenLatitude-LineDatabase.AllPointY[now]-LineDatabase.dy[i])/LatitudeScale*ScreenHeight);
							xx+=ScreenDeltaX;
							yy+=ScreenDeltaY;
							if(IsAlignLinesTag){
								String str=LineDatabase.getTitle(i);
								int size=setAlignTagVector(LineDatabase,str,now,(ScreenWidth/2/LongitudeScale)+(ScreenHeight/2/LatitudeScale));
								g_2d.setFont(new Font("黑体",0,size));
								for(int ii=0;ii<str.length();ii++){
									xx=(int)((AlignPosX[ii]-ScreenLongitude)/LongitudeScale*ScreenWidth);
									yy=(int)((ScreenLatitude-AlignPosY[ii])/LatitudeScale*ScreenHeight);
									xx+=ScreenDeltaX;
									yy+=ScreenDeltaY;
									if(!CheckInScreen(xx,yy)) continue;
									g_2d.drawString(str.substring(ii,ii+1),xx,yy);
								}
								DrawCount++;
								continue;
							}
							if(!CheckInScreen(xx-50,yy-50,xx+50,yy+50)) continue;
							DrawCount++;
							if(LineDatabase.isVertical[i]==false)
							g_2d.drawString(LineDatabase.getTitle(i), xx, yy);
							else{
								String str=LineDatabase.getTitle(i);
								for(int ii=0;ii<str.length();ii++){
									g_2d.drawString(str.substring(ii,ii+1), xx, yy+(((int)(tempsize*200))+5)*ii);
								}
							}
						}
						if(LineDatabase.LineHint[i].indexOf("[Info:Cache]")!=-1)
			...
			if(PolygonDatabaseFile!=null){
				int binary,choose,now,p1,p2;
				Line2D line;
				tempsize=0.20-LongitudeScale/(LongitudeEnd-LongitudeStart);
				tempsize=tempsize<=0?0:tempsize;
				g_2d.setFont(new Font("黑体",0,(int )(200*tempsize)));
				int DrawCount=0;
				for(int i=0;i<PolygonDatabase.PolygonNum;i++){
					...
					if((binary&Ox("1"))!=0){
						if((binary&Ox("1000"))!=0){//For Line----------------------------
							...
							if(p2==-1) p2=now;
							if(!CheckInAREA(PolygonDatabase.AllPointX[p1],PolygonDatabase.AllPointY[p1],PolygonDatabase.AllPointX[p2],PolygonDatabase.AllPointY[p2])){
								p1=p2;
								if(p1==now) break;
								continue;
							}
							int x1=(int)((PolygonDatabase.AllPointX[p1]-ScreenLongitude)/LongitudeScale*ScreenWidth);
							int y1=(int)((ScreenLatitude-PolygonDatabase.AllPointY[p1])/LatitudeScale*ScreenHeight);
							int x2=(int)((PolygonDatabase.AllPointX[p2]-ScreenLongitude)/LongitudeScale*ScreenWidth);
							int y2=(int)((ScreenLatitude-PolygonDatabase.AllPointY[p2])/LatitudeScale*ScreenHeight);
							x1+=ScreenDeltaX;
							y1+=ScreenDeltaY;
							x2+=ScreenDeltaX;
							y2+=ScreenDeltaY;
						...
						if((binary&Ox("100"))!=0){//For Point-------------------------
							...
							now=PolygonDatabase.PolygonHead[i];
							while(now!=-1){
								if(!CheckInAREA(PolygonDatabase.AllPointX[now],PolygonDatabase.AllPointY[now])){
									now=PolygonDatabase.AllPointNext[now];
									continue;
								}
								int xx=(int)((PolygonDatabase.AllPointX[now]-ScreenLongitude)/LongitudeScale*ScreenWidth);
								int yy=(int)((ScreenLatitude-PolygonDatabase.AllPointY[now])/LatitudeScale*ScreenHeight);
								xx+=ScreenDeltaX;
								yy+=ScreenDeltaY;
								if(CheckInScreen(xx,yy)){
								g_2d.fillOval(xx-3, yy-3, 6, 6);
						...
						if((binary&Ox("10"))!=0){//For Word--------------------------
							...
							if(IsAlignPolygonsTag){
								...
								int size=Screen.setCenterTagVector(PolygonDatabase,str,now,ScreenWidth/LongitudeScale);
								g_2d.setFont(new Font("黑体",0,size));
								for(int ii=0;ii<str.length();ii++){
									xx=(int)((Screen.CenterPosX[ii]-ScreenLongitude)/LongitudeScale*ScreenWidth);
									yy=(int)((ScreenLatitude-Screen.CenterPosY[ii])/LatitudeScale*ScreenHeight);
									xx+=ScreenDeltaX;
									yy+=ScreenDeltaY;
									if(!CheckInScreen(xx,yy)) continue;
									g_2d.drawString(str.substring(ii,ii+1),xx,yy);
							...
							xx=(int)((PolygonDatabase.AllPointX[now]+PolygonDatabase.dx[i]-ScreenLongitude)/LongitudeScale*ScreenWidth);
							yy=(int)((ScreenLatitude-PolygonDatabase.AllPointY[now]-PolygonDatabase.dy[i])/LatitudeScale*ScreenHeight);
							xx+=ScreenDeltaX;
							yy+=ScreenDeltaY;
							if(!CheckInScreen(xx-50,yy-50,xx+50,yy+50)) continue;
							DrawCount++;
							if(PolygonDatabase.isVertical[i]==false)
							g_2d.drawString(PolygonDatabase.getTitle(i), xx, yy);
							else{
								String str=PolygonDatabase.getTitle(i);
								for(int ii=0;ii<str.length();ii++){
									g_2d.drawString(str.substring(ii,ii+1), xx, yy+(((int)(tempsize*200))+5)*ii);
			...
			if(PointDatabaseFile!=null){
				...
				for(int i=0;i<PointDatabase.PointNum;i++){
					...
					if(DrawCount>VisualObjectMaxNum) break;
					if((binary&Ox("1"))!=0){
						if(binary<0) continue;
						if(!CheckInAREA(PointDatabase.AllPointX[i],PointDatabase.AllPointY[i])){
							continue;
						}
						if((binary&Ox("100"))!=0){//For Point-------------------------
							choose=(binary>>7)&Ox("111");
							g_2d.setColor(getChooseColor(choose));
							int xx=(int)((PointDatabase.AllPointX[i]-ScreenLongitude)/LongitudeScale*ScreenWidth);
							int yy=(int)((ScreenLatitude-PointDatabase.AllPointY[i])/LatitudeScale*ScreenHeight);
							xx+=ScreenDeltaX;
							yy+=ScreenDeltaY;
							if(!CheckInScreen(xx,yy)) continue;
							if(IsEngravePointShape)
								g_2d.drawRect(xx-3, yy-3, 6, 6);
							else g_2d.drawRect(xx,yy,1,1);
							DrawCount++;
						}
						if((binary&Ox("10"))!=0){//For Word--------------------------
							choose=(binary>>4)&Ox("111");
							...
							if(IsAlignPointsTag>0){
								g_2d.setFont(new Font("黑体",0,IsAlignPointsTag));
							}
							int xx=(int)((PointDatabase.AllPointX[i]-ScreenLongitude)/LongitudeScale*ScreenWidth);
							int yy=(int)((ScreenLatitude-PointDatabase.AllPointY[i])/LatitudeScale*ScreenHeight);
							xx+=ScreenDeltaX;
							yy+=ScreenDeltaY;
							if(!CheckInScreen(xx,yy)) continue;
							g_2d.drawString(PointDatabase.getTitle(i), xx+3, yy+3);
							DrawCount++;
						}
					}
				}
			...
			if(IsShowAlphaDistribution){
				if(Math.abs(LastScreenLongitude-ScreenLongitude)+
					...
					Math.abs(LastAlphaPercentScale-AlphaPercentScale)>1e-6){
			...
			if(IsTextArea2Visible){
				...
				g_2d.setColor(Color.red);
				g_2d.drawString(TextArea2Content, (int)(ScreenWidth*0.4)+2, ScreenHeight-9);
				g_2d.setColor(Color.green);
			}
			//----------------------------------------------------
		}
		...
		public void mousePressed(MouseEvent e){
			PressedX=e.getX();
			PressedY=e.getY();
			PressedLongitude=ScreenLongitude+(PressedX*LongitudeScale)/ScreenWidth;
			PressedLatitude=ScreenLatitude-(PressedY*LatitudeScale)/ScreenHeight;
			if(lock) return;
		}
		...
		boolean InsideSelectedRectangle(double x,double y){//判断是否在鼠标拖框的范围之内
			...
			if(xx<DrawRectX1) return false;
			if(xx>DrawRectX2) return false;
			if(yy<DrawRectY1) return false;
			if(yy>DrawRectY2) return false;
			return true;
		}
		public void mouseReleased(MouseEvent e){
			...
			if(NowPanel instanceof ExtendedToolPaneInterface){
				...
				double x=ScreenLongitude+e.getX()*LongitudeScale/ScreenWidth;
				double y=ScreenLatitude-e.getY()*LatitudeScale/ScreenHeight;
				((ExtendedToolPaneInterface)NowPanel).convey(PressedLongitude,PressedLatitude,x,y);
				return;
			}
			...
			if(lock && (NowPanel==CalibratePane) && (!CalibratePane.CanAdd)){
				...
				repaint();
			}else if(lock && ((NowPanel==LandMarkEditPane)||(NowPanel==BasicInfoPane))){
				CanDrawRect=false;
				int dx=e.getX()-PressedX;
				int dy=e.getY()-PressedY;
				if(Math.abs(dx)+Math.abs(dy)<10) {
				...
				DrawRect(PressedX,PressedY,e.getX(),e.getY());
				for(int i=0;i<GPSPoints.LandMarkNum;i++){
					if(InsideSelectedRectangle(GPSPoints.LandMarkLongitude[i],GPSPoints.LandMarkLatitude[i])){
			...
			}else if(lock &&((NowPanel==TaxiSearchPane)&&(TaxiSearchPane.CanShowTaxi))){
					CanDrawRect=false;
					int dx=e.getX()-PressedX;
					int dy=e.getY()-PressedY;
		...
		public void mouseClicked(MouseEvent e){
			...
			if(NowPanel==TwoPointPane){
				if(!TwoPointPane.CanChange) return;
				double x=ScreenLongitude+e.getX()*LongitudeScale/ScreenWidth;
				double y=ScreenLatitude-e.getY()*LatitudeScale/ScreenHeight;
				...
				if(!CalibratePane.CanAdd) return;
				double x=ScreenLongitude+e.getX()*LongitudeScale/ScreenWidth;
				double y=ScreenLatitude-e.getY()*LatitudeScale/ScreenHeight;
				NowPanel.setLongitude(x);
				NowPanel.setLatitude(y);
				...
				if(TaxiSearchPane.CanShowTaxi) return;
				double x=ScreenLongitude+e.getX()*LongitudeScale/ScreenWidth;
				double y=ScreenLatitude-e.getY()*LatitudeScale/ScreenHeight;
				XYCount=1;
				Xlist[0]=x;
				...
				TaxiSearchPane.TraceTheNearest.setEnabled(true);
				TaxiSearchPane.StopTrace.setEnabled(false);
				TaxiSearchPane.CenterLongitude.setText(java.lang.Double.toString(x));
				TaxiSearchPane.CenterLatitude.setText(java.lang.Double.toString(y));
				repaint();
			}else if(NowPanel==RouteSearchPane){
				if(RouteSearchPane.PathOnScreen) return;
				double x=ScreenLongitude+e.getX()*LongitudeScale/ScreenWidth;
				double y=ScreenLatitude-e.getY()*LatitudeScale/ScreenHeight;
				Xlist[XYCount]=x;
				Ylist[XYCount]=y;
				...
				if(RouteSearchPane.FocusSource){
					RouteSearchPane.SourceLongitude.setText(java.lang.Double.toString(x));
					RouteSearchPane.SourceLatitude.setText(java.lang.Double.toString(y));					
				}else{
					RouteSearchPane.TerminalLongitude.setText(java.lang.Double.toString(x));
					RouteSearchPane.TerminalLatitude.setText(java.lang.Double.toString(y));	
				}
				if(RouteSearchPane.FocusSource){
				...
				Ylist[0]=y;
				XYCount=1;
				LandMarkEditPane.LandMarkLongitude.setText(java.lang.Double.toString(x));
				LandMarkEditPane.LandMarkLatitude.setText(java.lang.Double.toString(y));
		...
		public void mouseDragged(MouseEvent e){
			...
			if(NowPanel instanceof ExtendedToolPaneInterface){
				...
				lastScreenLongitude=ScreenLongitude;
				lastScreenLatitude=ScreenLatitude;
				ScreenLongitude=PressedLongitude-e.getX()*LongitudeScale/ScreenWidth;
				ScreenLatitude=PressedLatitude+e.getY()*LatitudeScale/ScreenHeight;
				if(ScreenLongitude<LongitudeStart) ScreenLongitude=LongitudeStart;
				if(ScreenLongitude+LongitudeScale>LongitudeEnd) ScreenLongitude=LongitudeEnd-LongitudeScale;
				if(ScreenLatitude>LatitudeEnd) ScreenLatitude=LatitudeEnd;
				if(ScreenLatitude-LatitudeScale<LatitudeStart) ScreenLatitude=LatitudeStart+LatitudeScale;
				repaint();
				return;
			}
		...
		public void mouseMoved(MouseEvent e){
			...
			if(NowPanel instanceof ExtendedToolPaneInterface){
				((ExtendedToolPaneInterface) NowPanel).setLongitudeLatitude(x, y);
				return;
			}
		...
		void CenterLarger(double x0,double y0){//以给定的位置作为不动点放大屏幕视野
			if(LongitudeScale/(LongitudeEnd-LongitudeStart)<1.01/HighestRate) return;
			double CenterLongitude=ScreenLongitude+x0/ScreenWidth*LongitudeScale;
			double CenterLatitude=ScreenLatitude-y0/ScreenHeight*LatitudeScale;
		...
		boolean CheckScreen(){//检查屏幕坐标是否有异常
			if(ScreenLongitude<LongitudeStart) {initScreen();return false;}
			if(ScreenLatitude>LatitudeEnd) {initScreen();return false;}
			if(ScreenLongitude+LongitudeScale>LongitudeEnd) {initScreen();return false;}
			if(ScreenLatitude-LatitudeScale<LatitudeStart) {initScreen();return false;}
			if(LongitudeScale/(LongitudeEnd-LongitudeStart)<1.01/HighestRate/4) {initScreen();return false;}
			return true;
		}
	}
//Extended Definition---------------------------------------------------------------
...
//---------------------------------------------------------------
	...
	JMenuItem ChangeMapBackground,setDefaultMapBackground,ScreenLocationMicroDelta,ScreenLocationReset;
	JMenuItem ClearPointDBItem,AllElementInvisible,AllPointInvisible,AllLineInvisible,AllPolygonInvisible;
	JMenuItem EngravePointShape,ClearLineDBItem,ClearPolygonDBItem;
	
	boolean IsEngravePointShape=true;
...
//The Rest Work:
	...
	void MapInput(){//读取init文件，根据里面的内容打开各个相关文件
		try{
			...
			GPSPoints.CleanUp();
			if(TrafficFile!=null) GPSPoints.Input(TrafficFile,RoadDeltaLongitude,RoadDeltaLatitude);//路网数据流
			if(TaxiDir!=null) GPSPoints.TaxiStream(TaxiDir,GPSDeltaLongitude,GPSDeltaLatitude);//出租车数据流
			if(LandMarkFile!=null) GPSPoints.LandMarkStream(LandMarkFile);//地标数据流
			...
			if(TaxiTrajectoryDatabaseFile!=null){
				...
				System.out.println("DeltaX:"+GPSDeltaLongitude+"\nDeltaY:"+GPSDeltaLatitude);
				TaxiTrajectoryDatabase.DatabaseFileInput(TaxiTrajectoryDatabaseFile);
			}
	...
	public void actionPerformed(ActionEvent e){
		if(e.getSource()==OpenItem){
			...
			int state=FileDialog.showOpenDialog(this);
			if(state==JFileChooser.APPROVE_OPTION){
				Screen.image=this.getToolkit().createImage(FileDialog.getSelectedFile().toString());
				Screen.repaint();
			}
			...
			if(Screen.XYCount>0) Screen.XYCount--;
			Screen.repaint();
			...
			if(!ClockWizard.pic.lock){
				...
				if(k%5==0){
					if(TaxiSearchPane.CanShowTaxi) TaxiSearchPane.GetGPSInfo();
				}
			}
			}
	...
	public class ClockWizardClass extends JFrame implements ActionListener,ItemListener{
		//显示和调节时钟的窗口
...
//--------------------------------------------------------------------------------------------------------
	public class ClockComponent extends JPanel implements MouseListener,MouseMotionListener,MouseWheelListener{
		//显示和操纵时钟的面板
		...
		public void spur(){
			...
			b=minute;
			c=h*5+minute/12;
			secondLine.setLine(150+offsetx,150+offsety,(int)pointSX[a],(int)pointSY[a]);
			minuteLine.setLine(150+offsetx,150+offsety,(int)pointMX[b],(int)pointMY[b]);
			hourLine.setLine(150+offsetx,150+offsety,(int)pointHX[c],(int)pointHY[c]);
			repaint();
		}
		public void mouseClicked(MouseEvent e){
		}
		...
		public void mousePressed(MouseEvent e){
			...
			if(Math.abs(e.getX()-pointSX[a])+Math.abs(e.getY()-pointSY[a])<20) Catch=1;
			else if(Math.abs(e.getX()-pointMX[b])+Math.abs(e.getY()-pointMY[b])<20) Catch=2;
		}
		public void mouseReleased(MouseEvent e){
			if(!lock) return;
			if(Catch!=0){
				double l1=150+offsety;
				double l2=Math.sqrt((150+offsetx-e.getX())*(150+offsetx-e.getX())+(150+offsety-e.getY())*(150+offsety-e.getY()));
				double l3=Math.sqrt((150+offsetx-e.getX())*(150+offsetx-e.getX())+e.getY()*e.getY());
				double sita=Math.acos((l1*l1+l2*l2-l3*l3)/2/l1/l2)/Math.PI*180;
				if(e.getX()<150+offsetx) sita=360-sita;
				if(Catch==1){
					secondLine.setLine(150+offsetx,150+offsety,(int)pointSX[(int)(sita/6)],(int)pointSY[(int)(sita/6)]);
					ClockWizard.ShowSecond.setText(Integer.toString((int)(sita/6)));
				}else if(Catch==2){
					minuteLine.setLine(150+offsetx,150+offsety,(int)pointMX[(int)(sita/6)],(int)pointMY[(int)(sita/6)]);
					ClockWizard.ShowMinute.setText(Integer.toString((int)(sita/6)));
				}
				repaint();
			}
			Catch=0;
		...
		try{
			...
			if(ClockWizard.ShowMinute.isFocusOwner()){
				int temp=Integer.parseInt(ClockWizard.ShowMinute.getText())-e.getWheelRotation();
				temp=temp+6000;
				temp=temp%60;
				minuteLine.setLine(150+offsetx,150+offsety,(int)pointMX[temp],(int)pointMY[temp]);
				ClockWizard.ShowMinute.setText(Integer.toString(temp));
				repaint();
				return;
			}else if(ClockWizard.ShowSecond.isFocusOwner()){
				int temp=Integer.parseInt(ClockWizard.ShowSecond.getText())-e.getWheelRotation();
				temp=temp+6000;
				temp=temp%60;
				secondLine.setLine(150+offsetx,150+offsety,(int)pointSX[temp],(int)pointSY[temp]);
				ClockWizard.ShowSecond.setText(Integer.toString(temp));
				repaint();
				return;
			}
			int temp=Integer.parseInt(ClockWizard.ShowHour.getText())-e.getWheelRotation();
			temp=temp+12000;
			temp=temp%12;
			hourLine.setLine(150+offsetx,150+offsety,(int)pointHX[temp*5],(int)pointHY[temp*5]);
	...
	public class MapHandle implements MapControl{
		...
		public double MetertoLongitude(double Meter){
			return Meter/6371.0/1000.0/Math.PI*180.0/Math.cos((LatitudeStart+LatitudeEnd)/360.0*Math.PI);
		}
		...
		public double LongitudetoMeter(double Longitude){
			return Longitude/180.0*Math.PI*6371.0*1000.0*Math.cos((LatitudeStart+LatitudeEnd)/360.0*Math.PI);
		}
		...
		public void PointOutputTransform(){
			try{
			String FilterTag=JOptionPane.showInputDialog(null,"Choose the Filter Tag");
			BufferedWriter BOUT=new BufferedWriter(new OutputStreamWriter(new FileOutputStream(new File(FileDialog.getCurrentDirectory(),GetInternationalTimeSignature()+"Point.txt")),"UTF-8"));
			for(int i=0;i<PointDatabase.PointNum;i++){
				if((FilterTag==null)||(FilterTag.equals(""))||(PointDatabase.PointHint[i].indexOf(FilterTag)==-1)) continue;
				BOUT.write((i+1)+":<gml:Point srsName=\"EPSG:54004\" xmlns:gml=\"http://www.opengis.net/gml\"><gml:coordinates decimal=\".\" cs=\",\" ts=\" \">");
				BOUT.write(PointDatabase.AllPointX[i]+","+PointDatabase.AllPointY[i]+" </gml:coordinates></gml:Point>\n");
			}
			BOUT.flush();
			BOUT.close();
			JOptionPane.showMessageDialog(null,"Finished");
		...
		public void LineOutputTransform(){
			try{
			String FilterTag=JOptionPane.showInputDialog(null,"Choose the Filter Tag");
			BufferedWriter BOUT=new BufferedWriter(new OutputStreamWriter(new FileOutputStream(new File(FileDialog.getCurrentDirectory(),GetInternationalTimeSignature()+"Line.txt")),"UTF-8"));
			for(int i=0;i<LineDatabase.LineNum;i++){
				if((FilterTag!=null)&&(!FilterTag.equals(""))&&(LineDatabase.LineHint[i].indexOf(FilterTag)==-1)) continue;
				BOUT.write((i+1)+":<gml:LineString srsName=\"EPSG:54004\" xmlns:gml=\"http://www.opengis.net/gml\"><gml:coordinates decimal=\".\" cs=\",\" ts=\" \">");
				int temp=LineDatabase.LineHead[i];
				while(temp!=-1){
					BOUT.write(LineDatabase.AllPointX[temp]+","+LineDatabase.AllPointY[temp]+" ");
		...
		public void PointInputTransform(){
			...
			try{
				...
				if(state==JFileChooser.APPROVE_OPTION){
					...
					BOUT=new BufferedWriter(new OutputStreamWriter(new FileOutputStream(new File(FileDialog.getCurrentDirectory(),GetInternationalTimeSignature()+"_Point.txt")),"UTF-8"));
				}else return;
				while((s1=BIN.readLine())!=null){
					i=s1.indexOf(":");
					BOUT.write("[PointStart]-----------------------\n");
					BOUT.write("[Info:BasicLandMark][Title:"+s1.substring(0,i)+"]\n");
					BOUT.write("903\n");
					i=s1.indexOf("<",i+1);
					i=s1.indexOf("<",i+1);
					i=s1.indexOf(">",i+1);
		...
		void OtherFormatPointInput(File PointDatabaseFile){
			...
			try{
				BIN=new BufferedReader(new InputStreamReader(new FileInputStream(PointDatabaseFile),"UTF-8"));
				while((s1=BIN.readLine())!=null){
					if(s1.equals("-1")){
						PointDatabase.add(xx[0],yy[0],"[Info:ManualAdd][Title:]");
						count=0;
					}else{
						i=s1.indexOf(" ");
						j=s1.indexOf(" ",i+1);
						k=s1.indexOf(" ",j+1);
						double y=java.lang.Double.parseDouble(s1.substring(i+1,j));
						double x=java.lang.Double.parseDouble(s1.substring(j+1,k));
						xx[count]=x;
						yy[count]=y;
						count++;
					}
			...
			try{
				...
				while((s1=BIN.readLine())!=null){
					...
					while(i<s1.length()){
						...
						y=java.lang.Double.parseDouble(s1.substring(j+1,k));
						i=k+1;
		...
		public void LineInputTransform(){
			...
			try{
				...
				if(state==JFileChooser.APPROVE_OPTION){
					BIN=new BufferedReader(new InputStreamReader(new FileInputStream(FileDialog.getSelectedFile()),"UTF-8"));
					BOUT=new BufferedWriter(new OutputStreamWriter(new FileOutputStream(new File(FileDialog.getCurrentDirectory(),GetInternationalTimeSignature()+"_Line.txt")),"UTF-8"));
				}else return;
				while((s1=BIN.readLine())!=null){
					i=s1.indexOf(":");
					...
					while(i<s1.length()){
						...
						yy=java.lang.Double.parseDouble(s1.substring(j+1,k));
		...
		void OtherFormatLineInput(File LineDatabaseFile){
			...
			double[] yy=new double[100000];
			try{
				BIN=new BufferedReader(new InputStreamReader(new FileInputStream(LineDatabaseFile),"UTF-8"));
				while((s1=BIN.readLine())!=null){
					if(s1.equals("-1")){
						LineNum++;
					...
					try{
						...
						while((s1=BIN.readLine())!=null){
							...
							while(i<s1.length()){
								...
								k=s1.indexOf(" ",i);
								if(j<0) break;
								double x=java.lang.Double.parseDouble(s1.substring(i,j));
								double y=java.lang.Double.parseDouble(s1.substring(j+1,k));
								xx[count]=x;
								yy[count]=y;
								count++;
								i=k+1;
		...
		public void PendingTrap(){
			...
			for(int i=0;i<PolygonNum;i++){
				if(PolygonDatabase.PolygonHint[i].indexOf("[Info:Trap]")==-1) continue;
				ArrayList<Integer> res=LineRTree.Search(PolygonDatabase.GetMBRX1(i), 
						PolygonDatabase.GetMBRY1(i),
						PolygonDatabase.GetMBRX2(i),
						PolygonDatabase.GetMBRY2(i));
					for(int j:res) LineDatabase.LineHint[j]+="[Info:Trap]";
			}
			...
			for(int i=0;i<PolygonNum;i++){
				if(PolygonDatabase.PolygonHint[i].indexOf("[Info:Trap]")==-1) continue;
				ArrayList<Integer> res=PointRTree.Search(PolygonDatabase.GetMBRX1(i),
						PolygonDatabase.GetMBRY1(i),
						PolygonDatabase.GetMBRX2(i),
						PolygonDatabase.GetMBRY2(i));
					for(int j:res){
						if(!PolygonDatabase.CheckInsidePolygon(i,PointDatabase.AllPointX[j],PointDatabase.AllPointY[j])) continue;
			...
			for(int i=0;i<PolygonNum;i++){
				if(PolygonDatabase.PolygonHint[i].indexOf("[Info:Trap]")==-1) continue;
				ArrayList<Integer> res=PolygonRTree.Search(PolygonDatabase.GetMBRX1(i),
						PolygonDatabase.GetMBRY1(i),
						PolygonDatabase.GetMBRX2(i),
						PolygonDatabase.GetMBRY2(i));
					for(int j:res) PolygonDatabase.PolygonHint[j]+="[Info:Trap++]";
			}
		...
		//[ACMCUP2014]This function is to Warning the Crossing Line and mark the [Info:Delete]
		public void CheckDelete(){
			...
			for(int i=0;i<LineNum;i++){
				if(LineDatabase.LineHint[i].indexOf("[Info:LinkRegion]")==-1) continue;
			...
			//Check Head-----------------------------------------------------------------------
				...
				while(!res.isEmpty()){
					if(PolygonDatabase.CheckInsidePolygon(res.get(0),
						LineDatabase.AllPointX[temp],LineDatabase.AllPointY[temp])) break;
						res.remove(0);
					}
				if(res.isEmpty()){ LineDatabase.LineHint[i]+="[Info:Delete]";continue;}
			//Check Middle Point----------------------------------------------------------------
				temp=LineDatabase.AllPointNext[temp];
				res.clear();
				while(temp!=-1){
					if(LineDatabase.AllPointNext[temp]==-1) break;
					res=PolygonRTree.Search(LineDatabase.AllPointX[temp]-XStep,
							LineDatabase.AllPointY[temp]-YStep,
							LineDatabase.AllPointX[temp]+XStep,
							LineDatabase.AllPointY[temp]+YStep);
			...
			for(int i=0;i<LineNum;i++){
				...
				res=LineRTree.Search(LineDatabase.GetMBRX1(i),
						LineDatabase.GetMBRY1(i),
						LineDatabase.GetMBRX2(i),
						LineDatabase.GetMBRY2(i));
				while(!res.isEmpty()){
					if(LineDatabase.CheckCross(i,res.get(0))) break;
					res.remove(0);
				}
				if(!res.isEmpty()){LineDatabase.LineHint[i]+="[Info:Delete]";continue;}
			}
		...
		//[ACMCUP2014]This function is to clone the points lines and polygons in several rows and columns 
		public void MultipleData(int row,int col){
			...
			for(int i=0;i<row;i++){
				for(int j=0;j<col;j++){
					ChangeTitle("=============>>>>>>>>>>>>>Clone On Row["+i+"]/Col["+j+"]");
					if((i==0)&&(j==0)) continue;
					for(int k=0;k<PointNum;k++){
					//Clone the Points---------------------------------------------------------------
						PointDatabase.add(PointDatabase.AllPointX[k]+j*(LongitudeEnd-LongitudeStart),
								PointDatabase.AllPointY[k]+i*(LatitudeEnd-LatitudeStart),
								PointDatabase.PointVisible[k],PointDatabase.PointHint[k]);
					}
					...
					//Clone the Lines-----------------------------------------------------------------
						...
						tot=0;
						while(temp!=-1){
							tempx[tot]=LineDatabase.AllPointX[temp]+j*(LongitudeEnd-LongitudeStart);
							tempy[tot]=LineDatabase.AllPointY[temp]+i*(LatitudeEnd-LatitudeStart);
							tot++;
							temp=LineDatabase.AllPointNext[temp];
						}
						LineDatabase.add(tempx,tempy,tot,LineDatabase.LineHint[k]);
					}
					...
					//Clone the Polygons-----------------------------------------------------------------
						...
						tot=0;
						while(temp!=-1){
							tempx[tot]=PolygonDatabase.AllPointX[temp]+j*(LongitudeEnd-LongitudeStart);
							tempy[tot]=PolygonDatabase.AllPointY[temp]+i*(LatitudeEnd-LatitudeStart);
							tot++;
							temp=PolygonDatabase.AllPointNext[temp];
						}
						PolygonDatabase.add(tempx,tempy,tot,PolygonDatabase.PolygonHint[k]);
					}
				}
			}
			String Info="[LongitudeStart]"+LongitudeStart+"\n";
			Info+="[LongitudeEnd]"+(LongitudeStart+col*(LongitudeEnd-LongitudeStart))+"\n";
			Info+="[LatitudeStart]"+LatitudeStart+"\n";
			Info+="[LatitudeEnd]"+(LatitudeStart+row*(LatitudeEnd-LatitudeStart))+"\n";
		...
		public int getSecond(){
			return ClockWizard.pic.getHour()*3600+ClockWizard.pic.getMinute()*60+ClockWizard.pic.getSecond();
		}
		...
		public boolean AccurateInsideRectangle(double x0,double y0,double x1,double y1,double x2,double y2){
			if(x0<Math.min(x1,x2)) return false;
			if(x0>Math.max(x1,x2)) return false;
			if(y0<Math.min(y1,y2)) return false;
			if(y0>Math.max(y1,y2)) return false;
			return true;
		}
		public boolean InsideStretchRegion(double Stretch,double x0,double y0,double x1,double y1,double x2,double y2){
			if(x0<Math.min(x1,x2)-Stretch) return false;
			if(x0>Math.max(x1,x2)+Stretch) return false;
			if(y0<Math.min(y1,y2)-Stretch) return false;
			if(y0>Math.max(y1,y2)+Stretch) return false;
			return true;
		}
		...
		public void PolygonDatabaseAppend(String Hint){
			PolygonDatabase.add(Screen.ExtendedPointX,Screen.ExtendedPointY,Screen.ExtendedPointCount, Hint);
		}
		public void LineDatabaseAppend(String Hint){
			LineDatabase.add(Screen.ExtendedPointX,Screen.ExtendedPointY,Screen.ExtendedPointCount, Hint);
		}
		public void PointDatabaseAppend(String Hint){
			for(int i=0;i<Screen.ExtendedPointCount;i++)
				PointDatabase.add(Screen.ExtendedPointX[i],Screen.ExtendedPointY[i],Hint);
		}
		...
		@Override
		public double AccurateDistance(double x0, double y0, double x1,double y1) {
			// TODO Auto-generated method stub
	...
	public Color getChooseColor(int k){
		if(k==0) return Color.white;
		else if(k==1) return Color.red;
		else if(k==2) return Color.yellow;
		else if(k==3) return Color.blue;
		else if(k==4) return Color.green;
		else if(k==5) return Color.cyan;
		else if(k==6) return Color.pink;
		else if(k==7) return Color.orange;
	...
	public boolean CheckInAREA(double x,double y){
		if(x<AREALEFT) return false;
		if(x>AREARIGHT) return false;
		if(y>AREAUP) return false;
		if(y<AREADOWN) return false;
		return true;
	}
	public boolean CheckInAREA(double x1,double y1,double x2,double y2){
		if(Math.max(Math.min(x1,x2),AREALEFT)>Math.min(Math.max(x1,x2),AREARIGHT)) return false;
		if(Math.max(Math.min(y1,y2),AREADOWN)>Math.min(Math.max(y1,y2),AREAUP)) return false;
		return true;
	}
	...
	public void JPGOutput(double ScreenLongitudeStart,double ScreenLongitudeEnd,double ScreenLatitudeStart,double ScreenLatitudeEnd,int JPGWidth,int JPGHeight,int bold,String FileName){
		//----------------------------------------------------------------------------------------
		try{
		SetAREA(ScreenLongitudeStart,ScreenLongitudeEnd,ScreenLatitudeStart,ScreenLatitudeEnd);
		System.gc();
		BufferedImage JPGimage = new BufferedImage(JPGWidth,JPGHeight,BufferedImage.TYPE_INT_RGB);
		...
		double Xst=(ScreenLongitude-LongitudeStart)/(LongitudeEnd-LongitudeStart)*Screen.image.getWidth(this);
		double Yst=(LatitudeEnd-ScreenLatitude)/(LatitudeEnd-LatitudeStart)*Screen.image.getHeight(this);
		double Xlen=(LongitudeScale)/(LongitudeEnd-LongitudeStart)*Screen.image.getWidth(this);
		double Ylen=(LatitudeScale)/(LatitudeEnd-LatitudeStart)*Screen.image.getHeight(this);
		//将需要显示的经纬度范围转化为窗口界面中像素值
		g_2d.drawImage(Screen.image, 0, 0,ScreenWidth,ScreenHeight,(int)Xst,(int)Yst,(int)(Xst+Xlen),(int)(Yst+Ylen),this);
		}
		BasicStroke bs=new BasicStroke(2,BasicStroke.CAP_ROUND,BasicStroke.JOIN_ROUND);
		g_2d.setStroke(bs);
		g_2d.setColor(Color.green);
			...
			for(int i=0;i<LineDatabase.LineNum;i++){
				...
				if((binary&Ox("1"))!=0){
					...
					if((binary&Ox("10"))!=0){//For Word--------------------------
						...
						g_2d.setColor(getChooseColor(choose));
						now=LineDatabase.LineHead[i];
						if(!CheckInAREA(LineDatabase.AllPointX[now]-LongitudeScale*0.1,
								LineDatabase.AllPointY[now]-LatitudeScale*0.1,
								LineDatabase.AllPointX[now]+LongitudeScale*0.1,
								LineDatabase.AllPointY[now]+LatitudeScale*0.1)){
							continue;
						}
						int xx,yy;
						if(IsAlignLinesTag){
							String str=LineDatabase.getTitle(i);
							int size=Screen.setAlignTagVector(LineDatabase,str,now,(ScreenWidth/2/LongitudeScale)+(ScreenHeight/2/LatitudeScale));
							g_2d.setFont(new Font("黑体",0,size));
							for(int ii=0;ii<str.length();ii++){
								xx=(int)((Screen.AlignPosX[ii]-ScreenLongitude)/LongitudeScale*ScreenWidth);
								yy=(int)((ScreenLatitude-Screen.AlignPosY[ii])/LatitudeScale*ScreenHeight);
								g_2d.drawString(str.substring(ii,ii+1),xx,yy);
							}
							continue;
						}
						xx=(int)((LineDatabase.AllPointX[now]+LineDatabase.dx[i]-ScreenLongitude)/LongitudeScale*ScreenWidth);
						yy=(int)((ScreenLatitude-LineDatabase.AllPointY[now]-LineDatabase.dy[i])/LatitudeScale*ScreenHeight);						
						if(LineDatabase.isVertical[i]==false)
						g_2d.drawString(LineDatabase.getTitle(i), xx, yy);
						else{
							String str=LineDatabase.getTitle(i);
							for(int ii=0;ii<str.length();ii++){
								g_2d.drawString(str.substring(ii,ii+1), xx, yy+(bold+12)*ii);
							}
						}
					}
				}
			...
			for(int i=0;i<PolygonDatabase.PolygonNum;i++){
				...
				if((binary&Ox("1"))!=0){
					...
					if((binary&Ox("10"))!=0){//For Word--------------------------
						...
						g_2d.setColor(getChooseColor(choose));
						now=PolygonDatabase.PolygonHead[i];
						if(!CheckInAREA(PolygonDatabase.AllPointX[now]-LongitudeScale*0.1
								,PolygonDatabase.AllPointY[now]-LatitudeScale*0.1,
								PolygonDatabase.AllPointX[now]+LongitudeScale*0.1,
								PolygonDatabase.AllPointY[now]+LatitudeScale*0.1)){
							continue;
						}
						int xx,yy;
		...
		if(PointDatabaseFile!=null){
			...
			for(int i=0;i<PointDatabase.PointNum;i++){
				binary=PointDatabase.PointVisible[i];
				if((binary&Ox("1"))!=0){
					if(!CheckInAREA(PointDatabase.AllPointX[i],PointDatabase.AllPointY[i])) continue;
					if((binary&Ox("100"))!=0){//For Point-------------------------
						choose=(binary>>7)&Ox("111");
						g_2d.setColor(getChooseColor(choose));
						int xx=(int)((PointDatabase.AllPointX[i]-ScreenLongitude)/LongitudeScale*ScreenWidth);
	...
	public void JPGOutput(){
		...
		java.text.SimpleDateFormat df = new java.text.SimpleDateFormat("yyyy-MM-dd-HH-mm-ss");
		JPGOutput(LongitudeStart,LongitudeEnd,LatitudeStart,LatitudeEnd,Integer.parseInt(s1),Integer.parseInt(s2),Integer.parseInt(s3),
				(new File(ImageDir,"["+df.format(new Date())+"]")).toString());
	}
	public void JPGOutput(double LongitudeStart,double LongitudeEnd,double LatitudeStart,double LatitudeEnd){
		...
		s2=s2.substring(0,s2.indexOf(';'));
		java.text.SimpleDateFormat df = new java.text.SimpleDateFormat("yyyy-MM-dd-HH-mm-ss");
		JPGOutput(Math.min(LongitudeStart,LongitudeEnd),
					Math.max(LongitudeStart,LongitudeEnd),
					Math.min(LatitudeStart,LatitudeEnd),
					Math.max(LatitudeStart,LatitudeEnd),
				Integer.parseInt(s1),Integer.parseInt(s2),Integer.parseInt(s3),(new File(ImageDir,"["+df.format(new Date())+"]")).toString());
		}catch(Exception ex){Handle.SolveException(ex);}
	}
