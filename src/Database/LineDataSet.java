...
public class LineDataSet implements LineDatabaseInterface{
	...
	public double GetRealMeter(double st_longitude,double st_latitude,double en_longitude,double en_latitude){
		double cosin = Math.cos((st_latitude + en_latitude)/2/180*Math.PI);
		double delta_X = Math.abs(en_longitude - st_longitude)*cosin/180*Math.PI*6371*1000;
		double delta_Y = Math.abs(en_latitude - st_latitude)/180*Math.PI*6371*1000;
		return Math.sqrt(delta_X*delta_X + delta_Y*delta_Y);
	}
	public double GetRealMeter(int id) {
		if(id >= LineNum) return 0;
		if(id < 0) return 0;
		...
		if(ptr!=-1) {
			last_longitude = AllPointX[ptr];
			last_latitude = AllPointY[ptr];
			ptr=AllPointNext[ptr];
		} else return 0;
		while(ptr!=-1){
			totallength += GetRealMeter(last_longitude,last_latitude,AllPointX[ptr],AllPointY[ptr]);
			ptr=AllPointNext[ptr];
		}
		return totallength;
	}
	...
	public void add(double[] PointX,double[] PointY,int PointCount,String Hint){
		...
		LineVisible[LineNum]=DefaultVisible;
		isVertical[LineNum]=false;
		dx[LineNum]=0;
		dy[LineNum]=0;
		LineNum++;
	...
	public void DatabaseInit(){
		...
		PointUse=0;
		LineNum=0;
		if(LineHead == null) LineHead=new int[LineMaxNum];
		if(LineTail == null) LineTail=new int[LineMaxNum];
		if(LineVisible == null) LineVisible=new int[LineMaxNum];
		if(isVertical == null) isVertical=new boolean[LineMaxNum];
		if(dx == null) dx=new double[LineMaxNum];
		if(dy == null) dy=new double[LineMaxNum];
	...
	private void UnsafeHintUpdate(int k, String UpdateWhat) {
		...
		String[] OriginalKeyValue = LineHint[k].split("\\]\\[");
		...
		LineHint[k] = NewKeyValue.toString();
	}
	...
	public void PrimaryKeyValueUpdate(double x1, double y1, double x2, double y2, String PrimaryKey, String UpdateWhat){ /**线程安全*/
		while(!MapKernel.MapWizard.SingleItem.Set_DB_Read_Write_Lock(true, true));
		for(int i= LineNum-1;i>=0;i--){
			if(CheckInRegion(i, x1, y1, x2, y2) && LineHint[i].contains(PrimaryKey)) UnsafeHintUpdate(i, UpdateWhat);
		}
		while(!MapKernel.MapWizard.SingleItem.Set_DB_Read_Write_Lock(true, false));
	}
	...
	public Vector<HashMap<String, Object>> KeyValueQuery(double x1, double y1, double x2, double y2, String Info0,String Info1,String Info2,String Info3,String Info4){ /**线程安全*/
		...
		for (int i = LineNum -1; i>=0; i--){
			...
			if(CheckInRegion(i, x1, y1, x2, y2)){
				String IDHashCode = Integer.toString(System.identityHashCode(LineHint[i]));
				String Hint = LineHint[i];
	...
	public double GetRealMeterFromPointtoLine(int id, double x,double y) {/** 计算点到线段的真实最短距离*/
		int ptr=LineHead[id];
		int lastptr=ptr;
		...
		while(ptr!=-1){
			...
			xx = x1 + u * px;
			yy = y1 + u * py;
			dist =Math.min(dist,GetRealMeter(xx,yy,x3,y3));
			lastptr=ptr;
			ptr=AllPointNext[ptr];
		}
		return dist;
	}
	public double PointLineCheck(int id,double x,double y){ /** 计算点到线段的坐标最短距离*/
		int ptr=LineHead[id];
		int lastptr=ptr;
	    double x1,y1,x2,y2,x3,y3,px,py;
	    double som,u,xx,yy,dx,dy,dist;
	...
	public boolean CheckCross(int p,int q){ /* 检查线段是否有交点，但是q_ptr的赋值好像漏了重置为LineHead[q]*/
		if(p==q) return false;
