...
public class MapElementsEditorPaneClass extends ToolPanel implements ExtendedToolPaneInterface,ActionListener,ItemListener{
	...
	public MapElementsEditorPaneClass(){
		JLabel Title=new JLabel(MapKernel.MapWizard.LanguageDic.GetWords("地图元素编辑面板"));
	...
	public void SpecificProcess(){
		PointElement=new JRadioButton(MapKernel.MapWizard.LanguageDic.GetWords("选择添加地标点状地图元素"));
		PointElement.setOpaque(false);
		PointElement.setSelected(true);
		LineElement=new JRadioButton(MapKernel.MapWizard.LanguageDic.GetWords("选择添加连通线状地图元素"));
		LineElement.setOpaque(false);
		PolygonElement=new JRadioButton(MapKernel.MapWizard.LanguageDic.GetWords("选择添加多边形状地图元素"));
		...
		ConfirmLink=new JCheckBox(MapKernel.MapWizard.LanguageDic.GetWords("勾选则开始编辑，取消则不能编辑"));
		ConfirmLink.setOpaque(false);
		ConfirmLink.addItemListener(this);
		add(ConfirmLink);
		ShowConsecutiveLink=new JCheckBox(MapKernel.MapWizard.LanguageDic.GetWords("勾选则开始点点连接，取消则不连接"));
		ShowConsecutiveLink.setOpaque(false);;
		ShowConsecutiveLink.addItemListener(this);
		add(ShowConsecutiveLink);
		ShowHeadTailLink=new JCheckBox(MapKernel.MapWizard.LanguageDic.GetWords("勾选则开始首尾相连，取消则不相连"));
		...
		add(LineElement);
		add(PolygonElement);
		RemovePoint=new JCheckBox(MapKernel.MapWizard.LanguageDic.GetWords("选中时删除地标点状数据"));
		RemovePoint.setOpaque(false);
		add(RemovePoint);
		RemoveLine=new JCheckBox(MapKernel.MapWizard.LanguageDic.GetWords("选中时删除连接线状数据"));
		RemoveLine.setOpaque(false);
		add(RemoveLine);
		RemovePolygon=new JCheckBox(MapKernel.MapWizard.LanguageDic.GetWords("选中时删除多边形状数据"));
		RemovePolygon.setOpaque(false);
		add(RemovePolygon);
		CancelLastOne=new JButton(MapKernel.MapWizard.LanguageDic.GetWords("取消添加序列中最近点"));
		CancelLastOne.addActionListener(this);
		add(CancelLastOne);
		CancelAll=new JButton(MapKernel.MapWizard.LanguageDic.GetWords("取消添加序列中全部点"));
		CancelAll.addActionListener(this);
		add(CancelAll);
		Submit=new JButton(MapKernel.MapWizard.LanguageDic.GetWords("提交添加序列信息到数据库"));
		Submit.addActionListener(this);
		add(Submit);
		RollBack=new JButton(MapKernel.MapWizard.LanguageDic.GetWords("从数据库撤销上一次的提交"));
	...
	public void itemStateChanged(ItemEvent e) {
		if(e.getSource()==ConfirmLink){
			if(ConfirmLink.isSelected()){
				MainHandle.ChangeTitle(MapKernel.MapWizard.LanguageDic.GetWords("元素生成中"));
				MainHandle.PointEmpty();
				MainHandle.setPointVisible(true);
			}else{
				MainHandle.ChangeTitle(MapKernel.MapWizard.LanguageDic.GetWords("放弃了生成"));
				MainHandle.PointEmpty();
				MainHandle.setPointVisible(false);
			}
		}else if(e.getSource()==ShowConsecutiveLink){
			if(ShowConsecutiveLink.isSelected()){
				MainHandle.ChangeTitle(MapKernel.MapWizard.LanguageDic.GetWords("显示连线"));
				MainHandle.setPointConsecutiveLinkVisible(true);
			}else{
				MainHandle.ChangeTitle(MapKernel.MapWizard.LanguageDic.GetWords("隐去连线"));
				MainHandle.setPointConsecutiveLinkVisible(false);
			}
		}else if(e.getSource()==ShowHeadTailLink){
			if(ShowHeadTailLink.isSelected()){
				MainHandle.ChangeTitle(MapKernel.MapWizard.LanguageDic.GetWords("首尾相连"));
				MainHandle.setPointHeadTailLinkVisible(true);
			}else{
				MainHandle.ChangeTitle(MapKernel.MapWizard.LanguageDic.GetWords("首尾断开"));
				MainHandle.setPointHeadTailLinkVisible(false);
			}
		}else if(e.getSource()==ShowPointHint){
			if(ShowPointHint.isSelected()){
				MainHandle.ChangeTitle(MapKernel.MapWizard.LanguageDic.GetWords("显示顺序"));
	...
	public void convey(double x,double y){
		...
		}else MainHandle.ChangeTitle(MapKernel.MapWizard.LanguageDic.GetWords("没有开始构建元素，点击无效"));
	}
	public void convey(double x1,double y1,double x2,double y2){
		...
		MainHandle.setPointHintVisible(false);
		//ACMSIGSPACIAL2014
		int n=JOptionPane.showConfirmDialog(null,MapKernel.MapWizard.LanguageDic.GetWords("选择【是】删除，选择【否】撒点,【取消】则放弃"),
				MapKernel.MapWizard.LanguageDic.GetWords("删除或者撒点"),JOptionPane.YES_NO_CANCEL_OPTION);
		if(n==JOptionPane.YES_OPTION){
			...
			String str=JOptionPane.showInputDialog(null,MapKernel.MapWizard.LanguageDic.GetWords("输入随机点数量"),
					MapKernel.MapWizard.LanguageDic.GetWords("随机点数量"),JOptionPane.PLAIN_MESSAGE);
			int RandomCount=0;
			try{
				RandomCount=Integer.parseInt(str);
			}catch(Exception ex){
				JOptionPane.showMessageDialog(null,MapKernel.MapWizard.LanguageDic.GetWords("输入格式有误"));
				MainHandle.PointEmpty();
				MainHandle.setPointConsecutiveLinkVisible(false);
				MainHandle.setPointVisible(false);
				MainHandle.setPointHeadTailLinkVisible(false);
			...
			if(RandomCount<=0){
				JOptionPane.showMessageDialog(null,MapKernel.MapWizard.LanguageDic.GetWords("数量输入有误"));
				MainHandle.PointEmpty();
				MainHandle.setPointConsecutiveLinkVisible(false);
				MainHandle.setPointVisible(false);
				MainHandle.setPointHeadTailLinkVisible(false);
