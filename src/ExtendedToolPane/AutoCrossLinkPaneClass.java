...
public class AutoCrossLinkPaneClass extends MapKernel.ToolPanel implements ExtendedToolPaneInterface,ActionListener {
	...
	public AutoCrossLinkPaneClass(){
		PaneTitle=new JLabel("AutoCrossLinkPane");
		PaneTitle.setFont(new Font("华文新魏",Font.BOLD,25));
		add(PaneTitle);
		add(new JLabel("Cross-Cross Distance Upper Limit/m"));
		CrossCrossLimit=new JTextField(20);
		add(CrossCrossLimit);
		add(new JLabel("Cross-Skeleton Distance Upper Limit/m"));
		CrossRoadLimit=new JTextField(20);
		add(CrossRoadLimit);
		add(new JLabel("Cross-Skeleton Angle Upper Limit/360"));
		CrossAngleLimit=new JTextField(20);
		add(CrossAngleLimit);
		Confirm=new JButton("Confirm to Operate");
	...
	public void actionPerformed(ActionEvent arg0) {
		...
		try{
			...
			JOptionPane.showMessageDialog(null,"格式错误","格式错误",JOptionPane.WARNING_MESSAGE);
			return;
		}
