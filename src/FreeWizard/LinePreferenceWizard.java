...
import FreeWizard.LinePreferenceWizard.FacePic;
import MapKernel.MapControl;

public class LinePreferenceWizard extends JFrame implements FreeWizardInterface{
	...
	public FacePic(){
		...
		l0=new JLabel(MapKernel.MapWizard.LanguageDic.GetWords("【地理折线线路排布配置视窗】"));
		...
		IsWordVisible=new JCheckBox(MapKernel.MapWizard.LanguageDic.GetWords("在地图上显示字"));
		IsWordVisible.setOpaque(false);
		IsPointVisible=new JCheckBox(MapKernel.MapWizard.LanguageDic.GetWords("在地图上显示点"));
		IsPointVisible.setOpaque(false);
		IsLineVisible=new JCheckBox(MapKernel.MapWizard.LanguageDic.GetWords("在地图上显示线"));
		...
		add(PointPink);
		add(PointOrange);
		add(new JLabel(MapKernel.MapWizard.LanguageDic.GetWords("线色")));
		add(LineWhite);
		add(LineRed);
		add(LineYellow);
		add(LineBlue);
		...
		ReLocation=new JButton(MapKernel.MapWizard.LanguageDic.GetWords("设置文字位置"));
	...
	public void actionPerformed(ActionEvent e){
		if(e.getSource()==Hide){
			...
			if(IsUniversal.isSelected()){
				FreeWizard.LineDatabaseWizard TempView=MainHandle.getKernel().LineDatabaseView;
			...
			MainHandle.getKernel().LineHintReLocationView.convey(Index);
			MainHandle.getKernel().LineHintReLocationView.emerge();
		}
	}
	...
	public void Convey(int Index){
		this.Index=Index;
		KeyWord.setText(MainHandle.getLineDatabase().LineHint[Index]);
		Visible.setText(Integer.toString(MainHandle.getLineDatabase().LineVisible[Index]));
		int state=MainHandle.getLineDatabase().LineVisible[Index];
		int temp;
	...
	public void ProcessRow(int k){
		...
		StringBuffer Hint=new StringBuffer(MainHandle.getLineDatabase().LineHint[k]);
		...
		MainHandle.getLineDatabase().update(k,VisibleValue,new String(Hint));
	}
	int Index;
	@Override
	public void submerge() {
		// TODO Auto-generated method stub
		MainHandle.getKernel().LineDatabaseView.emerge();
		MainHandle.getKernel().LineDatabaseView.Pic.ProcessQuery();
